...
- Uses the declared crawl date window when judging absence: only previously captured entities that can be dated inside the later complete crawl's coverage are eligible to become missing.
- Excludes older and unknown-date records from negative inference instead of treating them as deleted.
...
scripts/                 ingestion, coverage, diff, verification, site build
...
For a date-limited complete crawl, the archive scopes missing detection to the declared `collectionLimit.cutoffDate` through the export date. A prior entity must have a resolvable content date inside that window before its absence counts. Unknown-date records remain preserved but are excluded from disappearance inference.
