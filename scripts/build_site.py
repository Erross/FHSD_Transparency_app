...
def main():
    ...
    for path in sorted((ROOT/'targets').glob('*/target.json')):
        cfg=read_json(path); tid=cfg['id']; state_path=ROOT/'data'/tid/'state.json.gz'; legacy=ROOT/'data'/tid/'state.json'; state=read_json(state_path) if state_path.exists() else (read_json(legacy) if legacy.exists() else initial_state(tid)); entities=sorted(state.get('entities',{}).values(),key=lambda e:(e.get('lastSeen',''),e.get('firstSeen','')),reverse=True); events=sorted(state.get('events',[]),key=lambda e:e.get('observedAt',''),reverse=True); snaps=sorted(state.get('snapshots',[]),key=lambda s:s.get('observedAt',''),reverse=True); public={'target':cfg,'summary':summary(state),'entities':entities,'events':events,'snapshots':snaps}; (target_out/f'{tid}.json').write_text(dumps(public),encoding='utf-8'); catalog.append({'id':tid,'displayName':cfg['displayName'],'description':cfg.get('description',''),'platform':cfg.get('platform',''),'sourceUrls':cfg.get('sourceUrls',[]),'summary':public['summary'],'latestSnapshot':snaps[0].get('observedAt','') if snaps else ''})
